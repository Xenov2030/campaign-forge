...
export function QuestDangerZone({ slug, questId, questName }: Props) {
  ...
  const run = async () => {
    const first = await confirm({
      ...
      description: `Se eliminará "${questName}" de forma permanente.`,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error ?? "No se pudo eliminar la misión");
      }
      toast.success("Misión eliminada");
      router.push(`/${slug}/quests`);
      router.refresh();
    } catch (err) {
