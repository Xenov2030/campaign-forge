...
import { Archive, X, Check, Loader2, Download, Trash2, ChevronLeft, ChevronRight, Package } from "lucide-react";
...
export function ItemVaultPicker({ campaignId }: { campaignId: string }) {
  ...
  const removeEntry = async (e: React.MouseEvent, id: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/item-vault/${id}`, { method: "DELETE" });
  ...
  const openModal = async () => {
    ...
    setPage(0);
    try {
      const res = await fetch("/api/item-vault");
  ...
  const importSelected = async () => {
    ...
    try {
      const res = await fetch("/api/items/import", {
        ...
        body: JSON.stringify({ campaignId, vaultItemIds: [...selected] }),
      });
      const data = await res.json() as { count?: number; error?: string };
      if (!res.ok) throw new Error(data.error);
      toast.success(`${data.count} objeto${data.count === 1 ? "" : "s"} traído${data.count === 1 ? "" : "s"} a la campaña`);
      setOpen(false);
      router.refresh();
    } catch (err) {
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-[var(--border-subtle)]">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-bold text-[var(--text-primary)]">Baúl — Objetos</h3>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {loading ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">Tu baúl de objetos está vacío.</p>
                  <p className="text-xs text-[var(--text-muted)] mt-1">
                    Guardá objetos desde su ficha para reutilizarlos en otras campañas.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {pageEntries.map((e) => {
                    ...
                    const color = RARITY_COLORS[e.rarity] ?? "#94a3b8";
                    return (
                      <div
                        key={e.id}
                        ...
                        >
                          {e.imageUrl ? (
                            <Image
                              ...
                              width={48}
                              height={48}
                              className="h-12 w-12 rounded-lg object-cover border-2 shrink-0"
                              style={{ borderColor: `${color}40` }}
                            />
                          ) : (
                            <div
                              className="h-12 w-12 rounded-lg border-2 flex items-center justify-center shrink-0"
                              style={{ backgroundColor: `${color}15`, borderColor: `${color}30` }}
                            >
                              <Package className="h-5 w-5" style={{ color: `${color}80` }} />
                            </div>
                          )}
                          <div className="min-w-0">
                            <p className="font-medium text-sm text-[var(--text-primary)] truncate">{e.name}</p>
                            {e.type && <p className="text-xs text-[var(--text-muted)] truncate">{e.type}</p>}
